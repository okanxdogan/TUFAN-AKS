...
## Üç dosya AYNI COMMIT'te güncellenmeli

1. **AKS** — `ESP_AKS/include/E22Regs.h`
   `E22_CRYPT_KEY=0x5A3CU`, `E22_CFG_CRYPT_H=0x5AU`, `E22_CFG_CRYPT_L=0x3CU`
   (bu commit'te YAPILDI). H/L ile key tutarlılığı `E22Config.cpp`'de
   `static_assert` ile derleme-zamanı bağlı.
2. **AKS** — `ESP_AKS/tools/e2e/contract.py`
   `E22_CRYPT_H=0x5A`, `E22_CRYPT_L=0x3C` (bu commit'te YAPILDI).
3. **UKS** — `TUFAN-UKS-TELEMETRY/UKS-Telemetry/Core/Inc/e22_regs.h`  ← **BU REPODA YOK, ELLE YAPILACAK**
   `#define E22_VAL_CRYPT_H  0x5A`
   `#define E22_VAL_CRYPT_L  0x3C`
   (Eski değer: her ikisi de `0x00`.) UKS'in E22 config yazım yolu bu
   register'ları modüle yazmalı (AKS tarafı `e22_buildWriteCommand` zaten
   CRYPT_H/L'yi yazıyor).
...
UKS bu makinede yoksa test toplanırken RuntimeError verir — CI kurulmadı,
elle çalıştırılır:

```
# UKS repo'yu göster ve E22 drift bekçisini çalıştır:
$env:TUFAN_UKS_REPO = 'C:\path\to\TUFAN-UKS-TELEMETRY'   # (pwsh)
