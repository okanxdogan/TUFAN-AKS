...
// Bu başlık saf kalsın diye SystemConfig.h buradan include EDİLMEZ;
// SystemConfig değerleriyle eşitlik kilidi (static_assert) iki başlığı
// birden gören VcuLogic.h'dedir.
...
// Şartname Bölüm 3, 6.e.iii: 55 uyarı / 70 kapanma, 15°C sabit aralık
// (eşitlik kilidi VcuLogic.h'de — bkz. yukarıdaki CRIT_C notu).
static constexpr int16_t BMS_TEMP_OVERTEMP_WARN_C = 55;       // >= => WARNING — SystemConfig.h BMS_WARN_MAX_TEMP_C ile hizalı

// ===========================================================================
// deci-mV ESDEĞERLERİ (VcuLogic.h / DeratingPolicy.h İÇİN)
// ===========================================================================
// GÜVENLİK-EŞİĞİ DÜZELTMESİ (2026-07-13): yukarıdaki dört eşik BmsAlgo.cpp
// içinde BmsPackData::cellVoltageMv[] (GERÇEK mV — main.cpp TEL_bmsCellVoltages[]
// üzerinden doldurur, o da CanParse'ta E015-E020 parse'ında /10 ile mV'ye
// çevrilir) ile karşılaştırılır — bu KULLANIM DOĞRU ve DEĞİŞTİRİLMEDİ.
//
// Ancak VcuLogic.h (hasWarningCondition/hasCriticalCondition) ve
// DeratingPolicy.h, hücre voltajı için FARKLI bir alan çifti kullanır:
// TEL_bmsCellVoltageMin/MaxDeciMv (0xE001'den CanParse::parseLbBmsE001 ile
// HİÇBİR /10 OLMADAN doğrudan yazılır — gerçek deci-mV, ~28000-40000
// aralığı). Bu iki katman aynı YUKARIDAKİ mV makrolarını kullanıyordu; sonuç
// gerçekçi bir hücre voltajıyla aşırı-gerilim dalının HER ZAMAN, düşük-gerilim
// dalının ise NEREDEYSE HİÇ tetiklenmemesiydi (bkz. Documents/
// Threshold_Ownership.md ve test_derating_policy.cpp — kapsamlı vaka
// analizi). DÜZELTME: VcuLogic.h/DeratingPolicy.h artık bu deci-mV
// eşdeğerlerini kullanır — AYNI fiziksel eşik (LiFePO4 2.50/2.80/3.55/3.65 V),
// yalnızca TEL_bmsCellVoltageMin/MaxDeciMv alanının ölçeğine (×10)
// dönüştürülmüş. Yukarıdaki mV sabitlerinden TÜRETİLİR — ileride biri
// değişirse ikisi birbirinden asla kopmaz.
static constexpr uint16_t BMS_CELL_UNDERVOLT_CRIT_DECI_MV = BMS_CELL_UNDERVOLT_CRIT_MV * 10;  // 25000
static constexpr uint16_t BMS_CELL_OVERVOLT_CRIT_DECI_MV = BMS_CELL_OVERVOLT_CRIT_MV * 10;    // 36500
static constexpr uint16_t BMS_CELL_UNDERVOLT_WARN_DECI_MV = BMS_CELL_UNDERVOLT_WARN_MV * 10;  // 28000
static constexpr uint16_t BMS_CELL_OVERVOLT_WARN_DECI_MV = BMS_CELL_OVERVOLT_WARN_MV * 10;    // 35500
